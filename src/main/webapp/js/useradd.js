...
$(function(){
	...
	userCode.bind("blur",function(){
		...
		$.ajax({
			type:"GET",//请求类型
			url:path+"/jsp/ucexist.do",//请求的url
	...
	/*userCode.bind("focus",function(){
		validateTip(userCode.next(),{"color":"#666666"},"* 用户账号长度必须是大于1小于10的字符",false);
	}).bind("blur",function(){
		if(userCode.val() != null && userCode.val().length > 1
				&& userCode.val().length < 10){
			validateTip(userCode.next(),{"color":"green"},imgYes+" 该账号可以使用",true);
		}else if(userCode.val().length > 10 || userCode.val().length <=1){
			validateTip(userCode.next(),{"color":"red"},imgNo+"用户账号长度必须是大于1小于10的字符",false);
		}else{
			validateTip(userCode.next(),{"color":"red"},imgNo+"用户编码是您登录系统的账号",false);
		}
	});*/
	...
	userName.bind("focus",function(){
		...
		}else if(userName.val().length > 10 || userName.val().length <=1){
			validateTip(userName.next(),{"color":"red"},imgNo+"用户名长度必须是大于1小于10的字符",false);
		}else{
			validateTip(userName.next(),{"color":"red"},imgNo+" 用户名不能为空",false);
		}
	});
